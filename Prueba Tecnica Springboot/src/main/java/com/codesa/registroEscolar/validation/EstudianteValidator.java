...
public class EstudianteValidator {
    ...
    public static void validar(EstudianteDTO dto, boolean esNuevo, EstudianteRepository repo) throws BusinessRuleException {
        ...
        if (dto.getNumeroMatricula() == null || dto.getNumeroMatricula().trim().isEmpty()) {
            throw new BusinessRuleException("1005", "La matricula no puede ser nula o vacia", HttpStatus.BAD_REQUEST);
        } else if (esNuevo && repo.existsByNumeroMatricula(dto.getNumeroMatricula())) {
            throw new BusinessRuleException("1005", "La matricula no puede repetirse", HttpStatus.BAD_REQUEST);
        }
